...
const DPTable = ({ items, capacity, step, solution, setSolution }) => {
  ...
  return (
    <div className="flex flex-col h-full">
      <h2 className="text-xl font-bold mb-4">0/1 Knapsack (Dynamic Programming)</h2>
      
      <div className="overflow-auto flex-1" ref={tableRef}>
        <table className="w-full border-collapse">
          <thead>
            <tr>
              <th className="p-2 bg-white/20 text-center">Weight →<br/>Item ↓</th>
              {Array(capacity + 1).fill().map((_, j) => (
                <th key={j} className="p-2 bg-white/20 text-center">{j}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {solution.dpTable.map((row, i) => (
              <tr key={i}>
                ...
                {row.map((cell, j) => {
                  const isHighlighted = solution.currentHighlight?.i === i && solution.currentHighlight?.j === j;
                  return (
                    <td 
                      key={j} 
                      className={`p-3 text-center relative transition-colors duration-300 ${
                        (i === items.length && j === capacity) ? 'bg-indigo-600/40' :
                        (i === 0 || j === 0) ? 'bg-white/5' : 'bg-white/10 hover:bg-white/20'
                      }`}
      ...
      {solution.currentHighlight && (
        <div className="mt-4 p-3 bg-white/20 rounded-lg">
          <p className="text-sm">
            {getTooltipText(solution.currentHighlight.i, solution.currentHighlight.j)}
          </p>
        </div>
      )}
    </div>
